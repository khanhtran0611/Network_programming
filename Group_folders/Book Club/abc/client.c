...
GtkWidget *sign_in_window, *sign_up_window, *main_window, *window;
...
{
    GtkEntry *signup_email;
    GtkEntry *signup_username;
    GtkEntry *signup_pass1;
    GtkEntry *signup_pass2;
    signup_window = GTK_WINDOW(gtk_builder_get_object(builder, "window_signup"));
    signup_email = GTK_ENTRY(gtk_builder_get_object(builder, "entry_email"));
    signup_username = GTK_ENTRY(gtk_builder_get_object(builder, "entry_username"));
    signup_pass1 = GTK_ENTRY(gtk_builder_get_object(builder, "entry_password"));
    signup_pass2 = GTK_ENTRY(gtk_builder_get_object(builder, "entry_password2"));
}
...
int main(int argc, char *argv[])
{
    ...
    GtkWidget *group_name_label, *members_listbox, *upload_button, *add_folder_button,
    ...
    if (gtk_builder_add_from_file(builder, "UI/group_members_screen.glade", &error) == 0)
    {
    ...
    if (gtk_builder_add_from_file(builder, "UI/sign_in.glade", &error) == 0)
    {
        // Xử lý lỗi nếu không tải được tệp
        g_printerr("Lỗi khi tải tệp UI: %s\n", error->message);
        g_clear_error(&error);
    ...
    if (gtk_builder_add_from_file(builder, "UI/sign_up.glade", &error) == 0)
    {
        // Xử lý lỗi nếu không tải được tệp
        g_printerr("Lỗi khi tải tệp UI: %s\n", error->message);
        g_clear_error(&error);
    ...
    if (gtk_builder_add_from_file(builder, "UI/test.glade", &error) == 0)
    {
        // Xử lý lỗi nếu không tải được tệp
        g_printerr("Lỗi khi tải tệp UI: %s\n", error->message);
        g_clear_error(&error);
